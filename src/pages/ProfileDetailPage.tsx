...
export function ProfileDetailPage() {
  ...
  const backUrl = `/?platform=${platform}`;
  const [profileData, setProfileData] = useState<ProfileDetailResponse | null>(
    null
  );
  const [loaded, setLoaded] = useState(false);

  // Reuse the shortlist state on the profile details page.
  ...
  if (!profileData) {
    return (
      <Layout title={`@${username}`}>
        ...
        <Link to={backUrl} className="text-blue-600 underline">
          Back to search
        </Link>
      </Layout>
    );
  ...
  const shortlisted = isShortlisted(user.user_id);
